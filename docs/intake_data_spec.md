...
## 5.5. Post-FNOL chase — Notification cadence for missing fields / documents

After the claim # is issued (per §1.5 gate check #9), two follow-up tracks may run in parallel:

- **Track A — Missing FNOL minimum fields**  → claim status = `PENDING_INCOMPLETE`. Chases the specific U1-U11 fields that are blank.
- **Track B — Missing supporting documents** → claim status = `OPEN` + flag `gbx_docs_outstanding = Y`. Chases per the §5 documents matrix.

Both tracks share the same owner: the **Notification Agent** — a Power Automate flow named `Chase_Missing_Items`, scheduled daily. Natural-language wording for each push is generated by the Explanation Agent (Azure OpenAI); the Notification Agent ships the message.

### Channel binding

Notifications use the **same channel the customer originally FNOL'd on** (Mobile App push, Web chat re-engagement, SMS, Teams chat, email reply chain). **Email is always added as a backup channel** because the policy always has an email on file.

| Original FNOL channel | Primary chase channel | Backup |
|---|---|---|
| Mobile App | Push notification | Email |
| Web Chat | Re-engagement banner in chat | Email |
| Microsoft Teams | Teams chat message | Email |
| SMS | SMS | Email |
| Email | Reply chain on existing thread | (none — email is primary) |

### Cadence

| Day | Action | Channel | Status change |
|---|---|---|---|
| **Day 0** | Claim # issued. First notification with full breakdown of what's missing. Secure upload link. | Original + email backup | `OPEN` or `PENDING_INCOMPLETE` |
| **Day 1** | Reminder if nothing received yet | SMS + email | unchanged |
| **Day 4** | Second push — softer tone, *"we're holding your claim for you"* | Original + email | unchanged |
| **Day 7** | Third push — clearer urgency, *"we need these to keep moving"* | Original + email + SMS | unchanged |
| **Day 10** | Adjuster manually contacts customer (phone or Teams) | Human handoff | `gbx_manual_contact_flag = Y` |
| **Day 14** | Claim status → `SUSPENDED`. Final notification. | All channels | `SUSPENDED` |
| **Day 30 (state-dependent)** | State-mandated closure rules trigger (varies 30-45 days by state) | Final notification | `CLOSED` |

State closure deadlines stored in `gbx_state_rule.gbx_closure_deadline_days`. The Notification Agent reads this at each chase iteration so the cadence respects state-specific windows.

### Notification content (every push includes)

1. **Claim #** front and centre
2. **Specific items outstanding** — bulleted, plain English
3. **Why each item is needed** — one short reason per item (so it's not opaque)
4. **Secure upload link** — Power Apps portal, JWT-authenticated for that claim ID, no second login required
5. **Reply shortcuts**:
   - `STATUS` → current claim state echoed back
   - `HUMAN` → transfer to live adjuster
   - `STOP` → opt out of chase notifications (claim status → `SUSPENDED` immediately, customer informed)
6. **Adjuster name + contact** — once assigned
7. **What happens if no response** — explicit timeline: Day 14 SUSPENDED, Day 30 CLOSED

### Channel-specific upload mechanics

| Channel | How customer uploads |
|---|---|
| Mobile App | Camera + secure upload widget; supports HEIC, JPG, PNG, PDF |
| Web Chat | Drag-and-drop or click-to-browse |
| Email | Reply with attachment; Power Automate parses the inbound mail, extracts attachments, links them to the claim |
| SMS | MMS attachment; Azure Communication Services receives, uploads to Blob |
| Teams | File attach in the open chat with the bot |

**All uploads land in the same place:** Azure Blob Storage → row added to `Document` table in Dataverse → Extraction Agent re-runs → `gbx_docs_outstanding` flag re-evaluated → if all items now received → status advances to the next agent in the pipeline.

### Audit logging — every notification + every reply

Every outbound notification logs to the `Communication` table:
- `claim_id`, `channel`, `direction = 'outbound'`, `agent = 'notification'`, `content_summary`, `delivery_status` (sent / delivered / read / failed), `timestamp`

Every customer reply logs to the `Communication` table:
- `claim_id`, `channel`, `direction = 'inbound'`, `content_summary`, `timestamp`, `attachments[]` (links to Blob)

This gives any auditor (or hackathon panel) a complete trail: who chased what, when, on which channel, and whether the customer responded.

### Wording differences between Track A and Track B

| Track | Example wording (generated by Explanation Agent) |
|---|---|
| **Track A** (missing FNOL fields) | *"We've started your claim 2026-CLM-04521 but still need a few details to keep it moving: date of loss, where it happened, and your vehicle's VIN. Reply here or tap the link to add them — takes about 2 minutes."* |
| **Track B** (missing docs) | *"Your claim 2026-CLM-04521 is being worked on. To keep it moving, please send us: the police report (Travis County case # if you have it), at least 3 damage photos (front, sides, rear), and a repair estimate. Tap the link to upload."* |

Both tracks use the same Notification Agent flow but feed different prompt templates to the Explanation Agent so the language matches what the customer is actually missing.
